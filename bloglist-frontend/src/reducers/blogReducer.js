...
import { createNotification } from "./notificReducer"
...
export const likeBlog = (id, blog) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(createNotification({ message: error.response.data.error, type: "error" },5))
    }
  }
}
...
export const createComment = (id, comment) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(
        createNotification(
          { message: `Comment ${comment} added`, type: "info" },5))
    } catch (error) {
      dispatch(
        createNotification(
